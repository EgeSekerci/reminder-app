...
const ReminderDetails = ({ reminder }) => {
    ...
    return (
        <div className="mb-4 mx-2">
            <button onClick={() => setOpenReminderUpdate(true)} className="block w-full p-6 bg-white border border-gray-200 text-[var(--primary-dark)] rounded-lg shadow transition-all hover:bg-gray-100 dark:bg-[var(--secondary-dark)] dark:border-zinc-800 dark:hover:bg-zinc-800 text-left">
                <div className="flex justify-between">
                    <h5 className="mb-2 text-base md:text-2xl font-bold text-[var(--primary-dark)] dark:text-[var(--primary-light)]">{reminder.title}</h5>
                    <PiTrashLight size={28} onClick={handleClick} className="text-[var(--primary-dark)] dark:text-[var(--primary-light)] transition-all hover:bg-[var(--secondary-light)] dark:hover:bg-[var(--primary-dark)]  cursor-pointer p-1 rounded-full" />
                </div>
